...
class TestCashierApi(TransactionCase):
    def setUp(self):
        ...
        self.cashier_role = self.env["ab_employee_access_sales_role"].sudo().create({
            "name": "Cashier Window Role",
            "code": "CASHIER-WINDOW",
            "allow_pos_screen": True,
            "allow_cashier_screen": True,
            "allow_sale": True,
        })
        ...
        self.cashier_profile = self.env["ab_employee_access"].sudo().create({
            ...
            "pos_allowed_store_ids": [(6, 0, [self.store.id])],
        })

    def _login_cashier(self, api):
        payload = api.cashier_pin_login(
            employee_id=self.cashier_employee.id,
            pin="1234",
            device_uid="CASHIER_TEST_DEVICE",
            device_name="Cashier Test Device",
        )
        return payload["session_token"]

    def test_pending_list_and_save_idempotent(self):
        api = self.env["ab_sales_cashier_api"].with_user(self.cashier_user)
        session_token = self._login_cashier(api)
        model_cls = type(api)
        pending_rows = [{
            "sth_id": 900001,
            "sth_flag": "P",
        ...
        with (
            ...
            payload = api.get_pending_invoices(limit=200, store_id=self.store.id, session_token=session_token)
    ...
    def test_invoice_snapshot_from_bconnect(self):
        ...
        with (
            ...
            snapshot = api.get_invoice_snapshot(900001, store_id=self.store.id, session_token=session_token)
    ...
    def test_wallets_for_store(self):
        ...
        with patch.object(
            ...
            payload = api.get_store_wallets(store_id=self.store.id, session_token=session_token)
    ...
    def test_cashier_access_required(self):
        ...
        with self.assertRaises(AccessError):
            api.get_pending_invoices(store_id=self.store.id, session_token="x")
        with self.assertRaises(AccessError):
            api.get_store_wallets(store_id=self.store.id, session_token="x")
