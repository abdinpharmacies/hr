...
class AbSalesCashierApi(models.TransientModel):
    ...
    @api.model
    def _default_store_id_for_cashier(self, profile=False):
        return self._safe_int(self.env["ab_sales_header"]._get_default_store_id(), 0)

    @api.model
    def _get_cashier_store_settings(self, profile=False, session=False):
        Store = self.env["ab_store"].sudo()
        stores = Store.search(self._store_domain_for_cashier(profile=profile))
        allowed_store_ids = stores.ids
        default_store_id = self._safe_int(self._default_store_id_for_cashier(profile=profile), 0)
        if default_store_id and default_store_id not in allowed_store_ids:
            default_store_id = 0
        return {
            "allowed_store_ids": [int(x) for x in allowed_store_ids],
    ...
    def _get_pos_hr_api(self):
        raise UserError(_("Cashier PIN login requires the Employee Access Sales module."))
    ...
    def _cashier_bootstrap_payload(self, session=False, profile=False):
        ...
        store_settings = self._get_cashier_store_settings(profile=profile, session=session)
        return {
            ...
            "require_pin_login": self._cashier_requires_pin_login(),
            "server_time": fields.Datetime.now(),
        }

    @api.model
    def _get_store_server(self, store):
        store = store if isinstance(store, models.BaseModel) else self.env["ab_store"].sudo().browse(int(store or 0))
        store = store.exists()
        if not store:
            return False
        target_ip = store.ip1
    ...
    @api.model
    def _coerce_store(self, store_id=None, required=False, require_connection=False, profile=False):
        Store = self.env["ab_store"].sudo()
        domain = self._store_domain_for_cashier(profile=profile)
    ...
    @api.model
    def _save_pending_invoice_in_bconnect(self, store, invoice_id, wallet_id, session=False):
        store_server = self._get_store_server(store)
        if not store_server:
            raise UserError(_("Store %s has no IP configured.") % (store.display_name,))
        current_flag = ""
            ...
            try:
                with conn.cursor(as_dict=True) as cur:
                    ...
                    if not rows:
                        ...
                        cashier_emp_id = self._get_cashier_employee_eplus_id(session=session)
    ...
    def get_cashier_bootstrap(self, session_token=False):
        ...
        return self._cashier_bootstrap_payload()
    ...
    def cashier_change_store(self, session_token, store_id):
        ...
        store = self._coerce_store(store_id=parsed_store_id, required=True, require_connection=False)
        payload = self._cashier_bootstrap_payload()
        payload.update({
    ...
    def save_pending_invoice(self, invoice_id, request_id=None, store_id=None, wallet_id=None, document_type="sale",
        ...
        if not invoice_id:
            raise UserError(_("Invalid invoice id."))
        session, __, store = self._resolve_cashier_context(
            session_token=session_token,
            store_id=store_id,
            require_connection=True,
        )
