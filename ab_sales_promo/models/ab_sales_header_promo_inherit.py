...
class AbSalesHeaderPromo(models.Model):
    ...
    def _program_is_effective(self, program):
        ...
        if not program:
            return False
        # Allow targeted callers (returns repricing) to honor an already-applied
        # source program even when current DB/store restrictions differ.
        force_program_effective = bool(self.env.context.get("force_program_effective"))
        if (not force_program_effective) and program.store_ids and (
                not self.store_id or self.store_id not in program.store_ids):
            return False
        if (not force_program_effective) and program.replica_db_ids:
    ...
    def _compute_header_numbers(self, cur=None):
        ...
        totals["total_bill_net"] = self._to_4dec(after)
        return totals
                ...
                f"""
                    UPDATE sales_trans_d
                     ...
                     WHERE sth_id = {PARAM_STR}
                       AND CHARINDEX({PARAM_STR}, ISNULL(col1, '')) = 0
                       AND itm_id IN ({placeholders})
                """,
                (marker, marker, int(sth_id), marker, *product_serials),
            )
        return lines_count
    ...
    @api.depends(
        ...
        'applied_program_ids.product_ids',
        'applied_program_ids.rule_min_qty',
        'applied_program_ids.rule_products_domain',
    ...
    def _auto_apply_programs(self):
        ...
        for h in self:
            ...
            if program.apply_disc_on == 'specific_products':
                meets_min_qty = h._specific_products_meets_min_qty(program)
            else:
                ...
                meets_min_qty = not scope or need_qty <= 0.0 or (total_scope_qty + 1e-8 >= need_qty)
    ...
    def _discount_for_program_on_product(self, program, product):
        ...
        Ignores taxes. Supports:
          ...
          - specific_products: pct * one eligible discounted unit
        ...
        if program.apply_disc_on == 'specific_products':
            if not self._specific_products_meets_min_qty(program):
                return 0.0
            return self._specific_products_discount_for_product(program, product, pct)
    ...
    def _specific_products_qty_state(self, program):
        self.ensure_one()
        trigger_scope = self._specific_products_trigger_products(program)
        discount_scope = program.disc_specific_product_ids
        trigger_qty = 0.0
        discount_qty = 0.0
        for line in self.line_ids:
            if not line.product_id or (line.qty or 0.0) <= 0:
                continue
            qty = self._line_qty_in_program_uom(line, program)
            if line.product_id in trigger_scope:
                trigger_qty += qty
            if line.product_id in discount_scope:
                discount_qty += qty
        return trigger_scope, discount_scope, trigger_qty, discount_qty

    def _specific_products_meets_min_qty(self, program):
        self.ensure_one()
        need_qty = float(program.rule_min_qty or 0.0)
        if need_qty <= 0.0:
            return True
        trigger_scope, discount_scope, trigger_qty, discount_qty = self._specific_products_qty_state(program)
        if not discount_scope:
            return False
        if not trigger_scope:
            return bool(discount_qty + 1e-8 >= need_qty)
        return bool(trigger_qty + 1e-8 >= max(need_qty - 1.0, 0.0) and discount_qty + 1e-8 >= 1.0)

    def _specific_products_missing_qty(self, program):
        self.ensure_one()
        need_qty = float(program.rule_min_qty or 0.0)
        if need_qty <= 0.0:
            return 0.0
        ...
        if not discount_scope:
            return need_qty
        if not trigger_scope:
            remainder = floor(discount_qty) % need_qty
            if remainder <= 1e-8:
                return 0.0
            return need_qty - remainder
        missing_trigger = max(0.0, max(need_qty - 1.0, 0.0) - floor(trigger_qty))
        missing_discount = max(0.0, 1.0 - floor(discount_qty))
        return missing_trigger + missing_discount

    def _specific_products_reward_unit(self, program):
        self.ensure_one()
        if not self._specific_products_meets_min_qty(program):
            return False, 0.0
        discount_scope = program.disc_specific_product_ids
        candidates = []
        for line in self.line_ids:
            if line.product_id not in discount_scope or (line.qty or 0.0) <= 0:
                continue
            units = int(floor(self._line_qty_in_program_uom(line, program) + 1e-8))
            if units <= 0:
                continue
            price = float(line.sell_price or 0.0)
            if price <= 0.0:
                continue
            candidates.append((price, line.product_id.id, line.product_id))
        if not candidates:
            return False, 0.0
        price, _product_id, product = sorted(candidates, key=lambda item: (item[0], item[1]))[0]
        return product, price

    def _specific_products_discount_for_product(self, program, product, pct):
        reward_product, reward_price = self._specific_products_reward_unit(program)
        if not reward_product or reward_product != product:
            return 0.0
        return reward_price * pct
    ...
    def _apply_promotion_to_lines(self):
        ...
        need_qty = float(program.rule_min_qty or 0.0)
        if program.apply_disc_on == 'specific_products':
            meets_min_qty = self._specific_products_meets_min_qty(program)
        else:
            meets_min_qty = True
            if need_qty > 0 and scope:
                ...
                meets_min_qty = total_scope_qty + 1e-8 >= need_qty
        if need_qty > 0 and not meets_min_qty:
    ...
    def _next_promo_suggestion(self):
        ...
        for program in self.applied_program_ids.filtered(lambda p: self._program_is_effective(p)):
            ...
            if program.apply_disc_on == 'specific_products':
                missing = self._specific_products_missing_qty(program)
                if missing:
                    messages.append(
                        _(
                            "Customer needs <b>%s</b> more unit(s) to unlock promotion <b>%s</b>.",
                            int(missing) if float(missing).is_integer() else missing,
