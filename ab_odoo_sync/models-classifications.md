Master Models
These should not be passive-mirrored from branches. Keep them report-owned or sync as controlled business_model
reference data. Branch facts should reference them by ID/code/serial.
...
- ab_product_company, ab_product_origin, ab_product_group, ab_product_tag, ab_scientific_group, ab_usage_causes,
  ab_usage_manner
...
These are worth syncing from branches because they are branch-local facts: time, store, employee, customer snapshot,
product, quantity, price, status, failures, or operational decisions.
...
- ab_employee_access_sales_operation_log, but exclude or aggregate heartbeat rows unless you need detailed
...
Do not sync these for normal data collection/analysis. They are configuration, transient UI state, cache, sync plumbing,
or report-generated artifacts.
...
- Transient/API models: ab_sales_cashier_api, ab_sales_cashier_close_wizard, ab_sales_cashier_close_wizard_line, POS API
  models, report wizards
- Sync/internal models: ab_odoo_sync_outbox, ab_odoo_sync_upload_source, ab_odoo_sync_upload_record,
  ab_odoo_sync_apply_profile, ab_odoo_sync_field_mapping, ab_odoo_sync_identity,

- Infra models: queue.job, queue.job.channel, queue.job.function, queue.job.lock

The core rule: sync branch facts, not global dimensions. ab_sales_header and ab_sales_line are high value; ab_product is
a master dimension; POS drafts/settings/printers/sync states are low value
