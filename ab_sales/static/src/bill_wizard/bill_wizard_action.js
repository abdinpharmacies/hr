...
class AbSalesBillWizardAction extends Component {
    ...
    setup() {
        ...
        onMounted(() => {
            const input = this.productSearchInputRef.el?.querySelector?.("input");
            input?.focus?.();
        });
    }
    ...
    _searchPayload(page = null) {
        ...
        const productIds = (this.state.filters.productSelection || [])
    ...
    productSelectionDomain() {
        const ids = (this.state.filters.productSelection || [])
            .map((row) => parseInt(row?.id || 0, 10) || 0)
            .filter((id) => id > 0);
