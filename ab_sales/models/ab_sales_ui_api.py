...
class AbSalesUiApi(models.TransientModel):
    ...
    @api.model
    def _prepare_print_html(self, html_content):
        ...
        if "<base " in lower_content:
            return content

        base_tag = f'<base href="{base_url}/"/>'
        head_open = lower_content.find("<head")
        if head_open != -1:
            head_end = content.find(">", head_open)
            if head_end != -1:
                return content[: head_end + 1] + base_tag + content[head_end + 1:]

        return f"<!DOCTYPE html><html><head><meta charset=\"utf-8\"/>{base_tag}</head><body>{content}</body></html>"
...
"""
    ...
    def bill_wizard_get_print_options(self):
        ...
        selected_format = (
                              selected_printer.paper_size if selected_printer and selected_printer.paper_size in (
                                  "pos_80mm", "a4")
    ...
    @api.model
    def _bill_wizard_resolve_selected_printer(self, printer_id=0, printer_name="", print_format="a4",
                                              allow_fallback=True):
    ...
    def _direct_print_linux_escpos_bin(self, html_content, printer_name=""):
        ...
        content = self._prepare_print_html(html_content)
    ...
    def _inventory_total_and_pos_balances_by_serial(self, product_serials, store_id=None):
        """
        Return tuple:
          - total_balance_by_serial: sum of store-specific rows
          - pos_balance_by_serial: selected store balance only (no fallback)
        """
        if not product_serials:
            return {}, {}
        ...
        if store_id:
            self.env.cr.execute(
                ...
                SELECT product_eplus_serial,
                       SUM(CASE WHEN store_id = %s THEN balance ELSE 0 END) AS pos_balance,
        ...
        # No selected store: return the all-stores aggregate only.
        self.env.cr.execute(
            """
    ...
    def get_products_promo_status(self, product_ids, store_id=None):
        """
        Return a dict mapping product_id to promo info for products that are in active promotions.
        """
        promo_data = {}
        if not product_ids:
            return promo_data
        ...
        # Find active promotions that include these products
        # ab_promo_program has fields: active, rule_date_from, rule_date_to, product_ids (many2many)
        # According to the model's _search_status method, for 'running' status both dates must be set and valid
        domain = [
            ('active', '=', True),
        ...
        # If store_id is provided, we might want to filter by store as well
        # This depends on your business logic
        # For now, we'll just get all active promotions for these products

        try:
            promo_programs = self.env['ab_promo_program'].search(domain)
            for program in promo_programs:
                for product in program.product_ids:
                    if product.id in product_ids:
                        # Store promo info
                        promo_data[product.id] = {
                            'promo_id': program.id,
                            'promo_name': program.display_name or '',
                        }
        except Exception as e:
            # Log error but don't break the flow
            _logger.warning("Error fetching promo data: %s", str(e))

        _logger.info("Promo data for products %s: %s", product_ids, promo_data)
        return promo_data
    ...
    def _search_products_by_partial_barcode(
        ...
        barcode_models = [
            model_name
            for model_name in ("ab_product_barcode", "ab_product_barcode_temp")
            if model_name in self.env.registry
        ]
        if not barcode_models:
            return []

        Product = self.env["ab_product"]
        fields_list = fields_list or self._safe_fields(
            "ab_product",
            [
                "name",
                "product_card_name",
        ...
        for row in rows:
            ...
            is_service_match = bool(row.get("is_service"))
    ...
    def search_products(
        ...
        def _run_search(search_domain, sql_filter=None):
            ...
            if want_pos_balance_only and store_id and not raw_query:
                ...
                self.env.cr.execute(
                    ...
                    SELECT
                        p.id,
                        i.balance AS pos_balance,
                        COALESCE(g.balance, 0) AS total_balance
                    FROM ab_product p
                    JOIN (
                        SELECT product_eplus_serial, SUM(balance) AS balance
                        FROM ab_sales_inventory
                        WHERE store_id = %s
                          AND balance > 0
                        GROUP BY product_eplus_serial
                    ) i ON i.product_eplus_serial = p.eplus_serial
                    LEFT JOIN LATERAL (
                        SELECT SUM(g.balance) AS balance
                        FROM ab_sales_inventory g
                        WHERE g.product_eplus_serial = p.eplus_serial
                          AND g.store_id IS NOT NULL
                          AND g.balance != 0
                        HAVING SUM(g.balance) > 0
                    ) g ON TRUE
                    WHERE p.active = TRUE
                      {extra_where}
                    ORDER BY p.name
        ...
        if raw_query:
            ...
            if len(rows) < 5:
                barcode_rows = self._search_products_by_partial_barcode(
                    barcode_query=raw_query,
                    limit=limit,
                    has_balance=want_balance_only,
