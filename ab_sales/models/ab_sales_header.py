...
DEBUG_PUSH_DELAY_SECONDS = float(config.get('ab_sales_debug_push_delay_seconds', 0) or 0)
...
class AbdinSalesHeader(models.Model):
    ...
    def _get_employee_delivery_id(self, employee=False):
        ...
        if employee:
            ...
            employee = self.employee_delivery_id.sudo().exists()[:1] if self.employee_delivery_id else Employee.browse()

        if employee and employee.costcenter_id and employee.costcenter_id.eplus_serial:
            employee_delivery_id = employee.costcenter_id.eplus_serial
        return employee_delivery_id
    ...
    def _compute_header_numbers(self, cur=None):
        ...
        return {
            ...
            'total_bill_after_disc': self._to_4dec(total_bill),
            'total_bill_net': self._to_4dec(total_net),
            'total_dis_per': 0,
            'total_des_mon': 0,
            'total_tax': 0.0,
        }
    ...
    def _maybe_delay_push_after_remote_commit(self):
        ...
        self.ensure_one()
        if DEBUG_PUSH_DELAY_SECONDS > 0:
            import time

            _logger.warning(
                "ab_sales debug push delay enabled for header %s: sleeping %.2fs",
                self.id,
                DEBUG_PUSH_DELAY_SECONDS,
            )
            time.sleep(DEBUG_PUSH_DELAY_SECONDS)
    ...
    def _bconnect_total_guard(self, cur, sth_id, tol=0.1):
        cur.execute(
        ...
        if abs(total_bill - total_lines) > float(tol):
            raise UserError(
                ...
                % (self._to_4dec(total_bill), self._to_4dec(total_lines))
            )
        ...
        for line in rec.line_ids:
            ...
            for batch in batches:
                ...
                params_d = (
                    ...
                    rec._to_4dec(batch["price_for_d"]),
                    rec._to_4dec(batch["cost_for_d"]),
                    rec._to_4dec(dis_mon),
                    rec._to_4dec(dis_per),
                    emp_code,
                    int(batch["itm_unit_val"]),
                    rec._to_4dec(batch["avg_cost_for_d"]),
    ...
    def _get_sales_trans_d_batches(self, cur, line):
        ...
        for source_id, exp_date_str, qty_to_take, batch_price, batch_cost, itm_nexist in _fifo_batches_for_line(
                ...
                is_service=line.product_id.is_service,
        ):
            if use_small_unit:
    ...
    def _insert_sales_deliv_info(self, cur, sth_id, emp_code):
        ...
        params = (
            ...
            tel,
            contact,
            rec._get_employee_delivery_id(rec.employee_delivery_id),
            cust_id_eplus,
            emp_code,
            None,
        )
    ...
    def _compute_store_server_online(self):
        for rec in self:
            rec.store_server_online = self.env['ab_sales_ui_api'].pos_store_status(store_id=rec.store_id)
    ...
    def cron_update_status_from_store(self):
        ...
        for store_id, serials in serials_by_store.items():
            ...
            store_server = self._get_store_server(store) if store else False
            if not store or not store_server:
                continue
            ...
            try:
                with self.connect_eplus(server=store_server, param_str=PARAM_STR, charset='CP1256') as conn:
                    with conn.cursor() as crx:
                        saved_serials = []
                        missing_serials = set()
                ...
                _logger.error(
                    ...
                    store.display_name, store_server, repr(ex),
                )
