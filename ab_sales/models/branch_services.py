...
class ApiSalesHeader(models.Model):
    ...
    def cron_update_status_from_store(self):
        ...
        for store in client._stores():
            headers = self.search([('store_id', '=', store.id), ('status', '=', 'pending'), ('eplus_serial', '>', 0)])
            serials = sorted(set(headers.mapped('eplus_serial')))
            for offset in range(0, len(serials), 200):
                batch = serials[offset:offset + 200]
                response = client._call(store, 'get_invoice_statuses', batch)
                for row in response['data']:
                    if row.get('invoice') not in batch or row.get('status') not in ('pending', 'saved'):
                        raise UserError(_('The branch returned invalid invoice status.'))
                    if row['status'] == 'saved':
                        headers.filtered(lambda h: h.eplus_serial == row['invoice']).write({'status': 'saved'})
        return True
