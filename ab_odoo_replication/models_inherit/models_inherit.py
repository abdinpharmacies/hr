...
class IgnoreWriteDateUpdate(models.AbstractModel):
    ...
    _REPLICATION_CTRL_PARAM = 'ab_odoo_replication_ctrl.enabled'
    _REPLICATION_CTRL_TRUTHY = {'1', 'true', 'yes', 'on'}

    @api.model
    def _replication_ctrl_enabled(self):
        value = self.env['ir.config_parameter'].sudo().get_param(
            self._REPLICATION_CTRL_PARAM,
            '0',
        )
        return str(value or '').strip().lower() in self._REPLICATION_CTRL_TRUTHY

    def _ignore_write_date_update(self, values):
        curr_write_date_by_id = {
            record['id']: record['write_date']
            for record in self.read(['write_date'])
        }
        res = super().write(values)
        if curr_write_date_by_id:
            self.env.cr.executemany(
                f"UPDATE {self._table} SET write_date = %s WHERE id = %s",
                [
                    (write_date, record_id)
                    for record_id, write_date in curr_write_date_by_id.items()
                ],
            )
...
class AbOdooReplicationOnlyAllowed(models.AbstractModel):
    ...
    def write(self, values):
        if self.env.context.get('replication') or self._replication_ctrl_enabled():
