...
class SalesDashboardReportTelemetry(models.Model):
    ...
    def get_fact_volume_analysis(self):
        ...
        queries = {
            ...
            "daily_user_facts": (("total_rows", "oldest_date", "newest_date", "distinct_employee_eplus_id_count", "distinct_store_eplus_id_count"), "SELECT COUNT(*), MIN(report_date), MAX(report_date), COUNT(DISTINCT employee_eplus_id), COUNT(DISTINCT store_eplus_id) FROM ab_sales_dashboard_daily_user_fact"),
            ...
            "coverage": (("legacy_sync_coverage_row_count", "fact_coverage_row_count", "store_coverage_count", "collection_coverage_count", "user_coverage_count", "item_coverage_count"), "SELECT (SELECT COUNT(*) FROM ab_sales_dashboard_sync_coverage), COUNT(*), COUNT(*) FILTER (WHERE fact_type = 'store'), COUNT(*) FILTER (WHERE fact_type = 'collection'), COUNT(*) FILTER (WHERE fact_type = 'user'), COUNT(*) FILTER (WHERE fact_type = 'item') FROM ab_sales_dashboard_fact_coverage"),
