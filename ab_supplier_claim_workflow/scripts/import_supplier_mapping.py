...
def _find_supplier(Supplier, name):
    ...
    for s in Supplier.search([]):
        ...
        has_code = bool(s.code and s.code.startswith('1-'))

        if not has_strong_rel:
            if len(common) < 2 and coverage < 0.6:
                continue
...
def import_supplier_mapping(env, filepath='/home/abdin_02/Downloads/بيانات موردين.xlsx'):
    ...
    stats = {
        'classified': 0, 'skipped_no_match': 0, 'skipped_invalid': 0,
    }

    for row in range(2, ws.max_row + 1):
        supplier_name = ws.cell(row, 2).value
        tax_raw = ws.cell(row, 3).value
        section_raw = ws.cell(row, 4).value
        region_raw = ws.cell(row, 5).value

        if not supplier_name or not tax_raw or not region_raw:
        ...
        supplier = _find_supplier(Supplier, supplier_name)
        if not supplier:
            stripped = _strip_noise(supplier_name.strip())
            if stripped and stripped != supplier_name.strip():
                supplier = _find_supplier(Supplier, stripped)
        if not supplier:
            words = supplier_name.strip().split()
            for i in [0, -1]:
                if len(words) > 1:
        ...
        mapping = Mapping.search([('supplier_id', '=', supplier.id)], limit=1)
        if not mapping:
