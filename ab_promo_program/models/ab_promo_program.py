...
class AbPromoProgram(models.Model):
    ...
    def _compute_rule_text(self):
        for rec in self:
            ...
            if rec.apply_disc_on == 'incentives':
                rec.rule_text = f"Incentives"
            elif rec.apply_disc_on == 'cheapest_product':
            ...
            elif rec.apply_disc_on == 'specific_products':
                rec.rule_text = _(
                    f"Buy {rec.rule_min_qty - 1}, get {int(rec.disc_percent)}% off on specific product.")
            else:
