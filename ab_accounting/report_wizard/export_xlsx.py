...
class AccountStatementExcel(models.AbstractModel):
    _name = 'report.ab_accounting_account_statement_excel'
    _description = 'Bills Actions'
    _inherit = ['excel_writer', 'report.report_xlsx.abstract']

    def generate_xlsx_report(self, workbook, data, objs):
        data = objs[0].get_account_statement_data()
        if data:
