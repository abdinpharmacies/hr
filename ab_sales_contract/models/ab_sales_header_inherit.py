...
class AbSalesHeaderContract(models.Model):
    ...
    discount = fields.Float(
        ...
        compute="_compute_contract_amount_totals",
        store=True,
        readonly=True,
    )
    ...
    cust_pay = fields.Float(
        string="Customer Pay",
        compute="_compute_contract_payment_totals",
        readonly=True,
    )
    ...
    # -------------------------------------------------------------------------
    @api.model
    def _ab_contract_record_from_value(self, model_name, value):
    ...
    @api.model
    def _ab_contract_validate_store_pair(self, contract, store):
        if not contract or not store or not contract.allowed_store_ids:
            return
        if store in contract.allowed_store_ids:
            return
        raise ValidationError(
            _("Contract %(contract)s is not allowed for store %(store)s.") % {
                "contract": contract.display_name,
                "store": store.display_name,
            }
        )

    def _ab_contract_validate_store_allowed(self, vals=None, check_status=True):
        vals = vals or {}
        for rec in self:
            status = vals.get("status", rec.status)
            if check_status and status not in ("pending", "saved"):
                continue

            contract = rec.contract_id
            store = rec.store_id
            if "contract_id" in vals:
                contract = self._ab_contract_record_from_value("ab_contract", vals.get("contract_id"))
            if "store_id" in vals:
                store = self._ab_contract_record_from_value("ab_store", vals.get("store_id"))

            self._ab_contract_validate_store_pair(contract, store)
    ...
    def _validate_before_push(self):
        ...
        self._ab_contract_validate_store_allowed(check_status=False)
    ...
    # Totals
    # -------------------------------------------------------------------------
    def _ab_contract_base_total_values(self):
        self.ensure_one()

        if not self.contract_id:
            gross_total = sum(
                float(l.qty or 0.0) * float(l.sell_price or 0.0) for l in self.line_ids
            )
            return {
                "discount": 0.0,
                "total_after_discount": gross_total,
                "cust_pay": 0.0,
                "company_pay": 0.0,
            }

        discount = sum(self.line_ids.mapped("ab_contract_discount_amount"))
        total_after_discount = sum(self.line_ids.mapped("ab_contract_net_amount"))
        gross_total = sum(
            float(l.ab_contract_gross_amount or (float(l.qty or 0.0) * float(l.sell_price or 0.0)))
            for l in self.line_ids
        )
        base_cust_pay = sum(self.line_ids.mapped("ab_customer_amount"))
        base_company_pay = sum(self.line_ids.mapped("ab_company_amount"))
        ...
        if 0.0 < cap < gross_total:
            ...
            cap_discount = 0.0
            if gross_total > 0.0 and discount:
                cap_discount = discount * (cap / gross_total)
    ...
    def _ab_contract_total_values(self):
        self.ensure_one()
        totals = self._ab_contract_base_total_values()
        return self._ab_contract_adjust_total_values(totals) or totals
    ...
    def _compute_contract_amount_totals(self):
        for rec in self:
            totals = rec._ab_contract_total_values()
            rec.discount = totals["discount"]
            rec.total_after_discount = totals["total_after_discount"]
    ...
    def _compute_contract_payment_totals(self):
        for rec in self:
            totals = rec._ab_contract_total_values()
            rec.cust_pay = totals["cust_pay"]
            rec.company_pay = totals["company_pay"]

    def _compute_contract_totals(self):
        self._compute_contract_amount_totals()
        self._compute_contract_payment_totals()
    ...
    def _compute_header_numbers(self, cur=None):
        ...
        return {
            ...
            "total_bill": self._to_4dec(total_bill),
            "total_bill_after_disc": self._to_4dec(total_bill_after_disc),
            "total_bill_net": self._to_4dec(total_bill_net),
            "total_dis_per": self._to_4dec(total_dis_per),
            "total_des_mon": self._to_4dec(total_des_mon),  # self._to_4dec(total_des_mon), # تصفير آخر للقيمة احتياطا
            "total_tax": 0.0,
        }
    ...
    def _insert_sales_trans_h(self, cur, totals, emp_code, pc_name, bill_typ):
        ...
        cur.execute(
            ...
            (
                contract_serial,
                self._to_4dec(float(self.company_pay or 0.0)),
                insurance_name,
                insurance_number,
                int(sth_id),
            ),
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if status in ("pending", "saved") and vals.get("contract_id") and vals.get("store_id"):
                contract = self._ab_contract_record_from_value("ab_contract", vals.get("contract_id"))
                store = self._ab_contract_record_from_value("ab_store", vals.get("store_id"))
                self._ab_contract_validate_store_pair(contract, store)
        ...
        for rec in records:
            if rec.status in ("pending", "saved"):
                rec._ab_contract_validate_store_allowed()
            if rec.contract_id:
    ...
    def write(self, vals):
        if {"status", "contract_id", "store_id"} & set(vals):
            self._ab_contract_validate_store_allowed(vals=vals)
